#!/usr/bin/env bash
# =============================================================================
# macOS-php / php85 / install.sh — install prebuilt PHP 8.5 + nginx from
# GitHub Releases (no local compilation).
#
# Downloads the release bundle, verifies its SHA-256, extracts it to the prefix
# it was built for, then generates the machine-local bits: a self-signed TLS
# certificate and an nginx.conf for your document root.
#
# Usage:
#   ./install.sh                       # latest php8.5 release
#   ./install.sh php85-v1.2.3          # a specific tag
#   SERVER_NAME=php.local PORT=8543 DOCROOT=/path/to/site ./install.sh
#   MACOS_PHP_URL=https://.../bundle.tar.xz ./install.sh   # explicit bundle URL
#
...
PREFIX="${PREFIX:-/opt/php8.5}"
SERVER_NAME="${SERVER_NAME:-localhost}"
PORT="${PORT:-8543}"
DOCROOT="${DOCROOT:-$PREFIX/www}"
TAG="${1:-}"
# Asset selector — distinguishes php8.5 bundles from other builds in this repo.
ASSET_MATCH='php8\.5-macos-arm64'
...
resolve_url() {
  ...
  if [ -n "$TAG" ]; then
    ...
    # all releases (newest first) — pick the newest php8.5 asset
    api="https://api.github.com/repos/$REPO/releases"
  fi
  c "querying $api" >&2
  curl -fsSL "$api" \
    | grep -oE '"browser_download_url"[ ]*:[ ]*"[^"]*'"$ASSET_MATCH"'[^"]*\.tar\.(zst|xz)"' \
    | sed -E 's/.*"(https?:[^"]*)"/\1/' \
    | head -1
}

URL="$(resolve_url)"
[ -n "$URL" ] || die "Could not find a php8.5 .tar.xz/.tar.zst asset in the release(s)."
ASSET="$(basename "$URL")"
c "bundle: $ASSET"
